...
public class TokenAuthenticationHandlerTest{
  ...
  public void testIsTokenValid_expired(){
    ...
    String token = encrypter.encrypt("testuser:" + DateTime.now().plusHours(-50).getMillis() + ":" + DateTime.now().plusHours(10).getMillis() + ":" + UUID.randomUUID().toString());
  ...
  public void testIsTokenValid_unexpired(){
    Encrypter encrypter = new Encrypter(Base64.decodeBase64(adminConfig.getKey()));
    String token = encrypter.encrypt("testuser:" + DateTime.now().getMillis() + ":" + DateTime.now().plusHours(10).getMillis() + ":" + UUID.randomUUID().toString());
    UserConfig userConfig = new UserConfig();
    userConfig.setToken(token);
    when(userDao.getUserConfig("testuser")).thenReturn(userConfig);
    when(adminDao.getAdminConfig()).thenReturn(adminConfig);
  ...
  public void testIsTokenValid_noexpiry(){
    ...
    Encrypter encrypter = new Encrypter(Base64.decodeBase64(adminConfig.getKey()));
    String token = encrypter.encrypt("testuser:" + DateTime.now().plusHours(-100).getMillis() + ":" + DateTime.now().plusHours(-100).getMillis() + ":" + UUID.randomUUID().toString());
    UserConfig userConfig = new UserConfig();
    userConfig.setToken(token);
    when(userDao.getUserConfig("testuser")).thenReturn(userConfig);
    when(adminDao.getAdminConfig()).thenReturn(adminConfig);
